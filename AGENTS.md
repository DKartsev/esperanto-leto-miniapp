...
### 8. **Интерфейс для мобильных устройств**
- Навигация фиксируется внизу экрана и учитывает безопасные области `env(safe-area-inset-*)`.
- Кнопки и карточки занимают всю ширину на небольших экранах и уменьшаются на больших.
- Ненужные элементы скрываются с помощью медиа‑запросов для упрощения интерфейса.

### 9. **Элементы управления состоянием**
- Компоненты используют React хуки `useState()` и `useEffect()` для хранения прогресса и загрузки данных.
- Хук `useEsperantoData()` управляет состоянием глав, разделов и прогресса пользователя.

### 10. **Интерактивность**
- Все кнопки и карточки имеют плавные эффекты наведения и нажатия (увеличение и изменение градиента).
- Прогресс-бары заполняются классами `.progress-track` и `.progress-fill`, которые анимируются при изменении данных.

### 11. **Управление данными с Supabase**
- Данные глав загружаются через `supabase.from('chapters')`, а разделов — через `supabase.from('sections')`.
- Результаты тестов сохраняются в таблицу `test_results` с помощью Supabase API.
