...
Приложение использует Telegram Login. После запуска в Telegram WebApp данные пользователя
(ID и имя пользователя) преобразуются в UUID и сохраняются в таблицы Supabase через RPC
функцию `create_user_from_telegram`. Если такой пользователь уже есть, новый не создаётся.

> **Note**: Из-за этой логики таблица `profiles` больше не привязана внешним ключом к `auth.users`. Обновите базу с помощью миграций `20250719200000_fix_profiles_fk.sql` и `20250721101500_add_telegram_id_to_profiles.sql`.

### Структура базы данных

#### Таблица `profiles`
- Профили пользователей с дополнительной информацией
- Не связана напрямую с `auth.users`, что упрощает регистрацию через Telegram
