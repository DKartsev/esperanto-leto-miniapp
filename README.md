...
Tailwind CSS 4 и Vite используют возможности Node 20.
Убедитесь, что установлен Node.js `>=20.19.0`.
При использовании более старых версий (например, Node 16 или 18)
могут возникать ошибки вроде `crypto.hash is not a function` или
трассировки из `@tailwindcss/postcss`.
...
1. Создайте файл `netlify.toml` со следующими настройками:
...
[[redirects]]
  from = "/images/*"
  to = "/dist/images/:splat"
  status = 200
```

2. В панели Netlify укажите переменные окружения из `.env`.
3. Нажмите **Trigger deploy → Deploy site** для публикации.
