...
Для корректной работы используйте Node.js `>=20.19.0`.
...
npm start
```
Команда собирает проект и запускает одновременно:
- 🤖 Telegram бот
- 🌐 Веб-приложение (предпросмотр на http://localhost:4173)
...
### Команды управления:
```bash
npm start           # Сборка и запуск всей платформы
npm run start:dev   # Режим разработки (бот + веб)
